...
compile("NB_lobster_transect_interaction.cpp")
compile("NB_lobster_transect_full.cpp")
...
res <- res[res$year %in% 2003:2018, ]
res <- res[!(res$region %in% c("GA1", "PH", "TB", "RO")), ]

# Create variable factor labels:
years <- sort(unique(res$year))
res$year.label   <- match(res$year, years)
#res$region.label <- match(res$region, c("PV", "GA1", "AB1", "CR", "PH", "TB", "NG", "RI", "CG", "SH", "RO", "MC", "FH", "TR"))
regions <- c("PV", "CR", "NG", "RI", "CG", "SH", "MC", "FH", "TR")
res$region.label <- match(res$region, regions)
res$class.label <- res$year.class + 1
#res$region.label <- match(res$region, c("PV", "GA1", "AB1", "CR", "PH", "TB", "NG", "RI", "CG", "SH", "RO", "MC", "FH", "TR"))
res$transect.label <- match(res$transect, sort(unique(res$transect)))
...
data <- list(y = res$n,
             ...
             cohort = res$class.label,
             transect = res$transect.label) 

# Initialize full parameter vector:
parameters <- list(alpha = 0,  
                   year_effect = rep(0, length(unique(data$year))),
                   region_effect = rep(0, length(unique(data$region))),
                   cohort_effect = rep(0, length(unique(data$cohort))),
                   year_region_effect = rep(0, length(unique(data$year)) * length(unique(data$region))),          
                   year_cohort_effect = rep(0, length(unique(data$year)) * length(unique(data$cohort))),           
                   region_cohort_effect = rep(0, length(unique(data$region)) * length(unique(data$cohort))),  
                   year_region_cohort_effect = rep(0, length(unique(data$year)) * length(unique(data$region)) * length(unique(data$cohort))),
                   transect_effect = rep(0, length(unique(data$transect))), 
                   log_sigma_year = 0,               
                   log_sigma_region = 0,             
                   log_sigma_cohort = 0,
                   log_sigma_year_region = 0,      
                   log_sigma_year_cohort = 0, 
                   log_sigma_region_cohort = 0,    
                   log_sigma_year_region_cohort = 0,
                   log_sigma_transect = 0,
                   log_r = 0)
                                   
# Create TMB object:
obj <- MakeADFun(data = data[data.cpp("NB_lobster_transect_additive.cpp")], 
                 parameters = parameters[parameters.cpp("NB_lobster_transect_additive.cpp")],
                 random = c("year_effect", "region_effect", "cohort_effect", "transect_effect"),
                 DLL = "NB_lobster_transect_additive")
