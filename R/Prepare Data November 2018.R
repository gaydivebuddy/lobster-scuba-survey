...
x <- read.table("Lobster Transect Data 2018.csv", header = TRUE, sep = ",", colClasses = "character", stringsAsFactors = FALSE)
...
x$mallet[which(x$year.class == 6 & x$mallet == 1)] <- 0
...
for (i in 0:6){
   ...
   index <- gulf::match(res[vars], temp[vars])
...
#swept.area$n <- aggregate(list(n = x$length), by = x[key], function(x) sum(!is.na(x)))$n

sort(swept.area[swept.area$year == 2018, ], by = c("region", "transect"))

# Swept area plot:
m <- kronecker(1:6, matrix(1, nrow = 3, ncol = 4))
...
par(mar = c(0, 0, 0, 0))
for (i in 2013:2018){
   dbarplot(table(swept.area$swept.area[swept.area$year == i]), xlim = c(0, 400), xaxt = "n")
   if (i == 2018) axis(1)
   text(20, par("usr")[3] + 0.875 * diff(par("usr")[3:4]), i, cex = 1.25)
   if (i == 2015) mtext("Number of transects", 2, 2.5, at = 0, cex = 1.5)
}
mtext("Swept area (m2)", 1, 2.5, cex = 1.5)
...
aggregate(res$transect, by = res[c("region", "year")], function(x) length(unique(x)))
